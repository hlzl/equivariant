...
def _generalized_he_init_variances(basisexpansion: BasisExpansion):
    ...
    for w, attr in enumerate(basis_info):
        i, o = attr["in_irreps_position"], attr["out_irreps_position"]
        in_irrep, out_irrep = attr["in_irrep"], attr["out_irrep"]
        vars[w] = 1.0 / math.sqrt(inputs_count[o] * basis_count[(in_irrep, o)])
...
def generalized_he_init(
    tensor: torch.Tensor, basisexpansion: BasisExpansion, cache: bool = False
):
    r"""
...
def deltaorthonormal_init(tensor: torch.Tensor, basisexpansion: BasisExpansion):
    ...
    If no 'radius' attribute is present in the `basisexpansion.get_basis_info()`, it is assumed the parameters
    parametrize a :class:`~nn.Linear` layer.
    ...
    for irrep, count in counts.items():
        assert same_content([len(x) for x in count.values()]), [
