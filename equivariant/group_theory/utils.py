...
__all__ = ["psi", "chi", "psichi"]


def psi(theta: float, k: int = 1, gamma: float = 0.0):
    r"""
...
def psichi(theta: float, s: int, k: int = 1, gamma: float = 0.0):
    r"""
...
def cycle_isclose(a, b, S, rtol=1e-9, atol=1e-11):
    ...
    close_0 = math.isclose(d, 0.0, rel_tol=rtol, abs_tol=atol) and d >= 0.0
